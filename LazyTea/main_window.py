...
class MainWindow(QWidget):
    ...
    def load_decoration_image(self) -> None:
        ...
        try:
            ...
            try:
                bg_container = importlib.resources.files(__package__).joinpath("resources", "bg")
                if bg_container.is_dir():
                    for item in bg_container.iterdir():
                        if item.is_file() and item.name.lower().endswith(('.jpg', '.png', '.jpeg')):
                            image_files.append(item)
            except Exception as e:
                logger.debug(f"Could not list resources via importlib.resources: {e}. Will try fallback.")
